...
daily:
  ...
  refresh-cost: 1000.0          # 刷新费用（自动选货币；0 = 免费）
...
刷新费用按 **金币 → 点券 → 经验** 的顺序自动挑选一个可用货币：

| 顺序 | 货币 | 何时使用 |
|---|---|---|
| 1 | **金币** | 装了 Vault + 任一经济插件（EssentialsX、CMI 等） |
| 2 | **点券** | 装了 PlayerPoints |
| 3 | **经验** | 以上都没有——原版资源，**任何服务端都可用** |
...
- 玩家执行 `/ptx refresh` 时，提示里会说明**实际扣的是哪种货币**，
  例如「本次刷新消耗 1,000 金币」或「本次刷新消耗 1000 经验」
- 余额不足时的提示同样带货币名与当前余额，例如「经验不足，需要 1000（当前 320）」
- 设为 `0` 则免费刷新，此时完全不涉及货币
- 管理员用 `/ptxa resetdaily` 重置**不扣费也不消耗次数**（它是排障工具，不是消费入口）

> 早期版本有「任务币」内建货币与手动的刷新货币选项，现已移除：
> 货币选择完全自动，插件不再引入玩家需要额外理解的第二套货币。
