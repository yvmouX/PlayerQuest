...
- **游戏内指令与编辑器**：`/ptxa` 系列（适合脚本化改动）与 `/ptxa menu`（新建 / 编辑 / 删除任务，见 [命令](commands)）；
- **`quests/` 目录下的 YAML 文件**：**只读**来源，适合随插件发布、进版本控制（见下节）。

第一条**保存即生效**（不需要重启，也不需要 reload）。

要 diff、进版本控制或跨服搬运时，把库里的定义**手工整理成 `quests/` / `presets/` 下的 YAML 文件**
（字段名与文件格式完全一致，见下节）。
...
切换后端**不会自动搬运已有数据**：SQLite 与 MySQL 之间要换，得手工把定义搬过去。
...
**文件夹空着时插件会铺一份示例进去**（12 个任务 + 11 个预设，id 统一是 `example_` 前缀）。
示例随插件发布（在插件 jar 内的 `quests/`、`presets/` 资源里），只在目录空着时复制一份过来，
让你直接看到格式、复制一份改改就能用。它们是**只读**的：改文件才生效，游戏内命令只改数据库。
不想要示例：删掉文件即可——**只要目录里还有任何一个 YAML，插件就不会再补**；
想彻底不读文件就设 `definitions.read-files: false`。

> 数据库里**不再**播种任何示例（早期版本会把一套 `example_*` 写进库里）：
> 定义要么来自文件，要么来自游戏内新建的内容。

`quests/example_daily_mine.yml`（文件名就是 id，因此正文里不写 `id`）：
...
- **可以引用预设**（`preset: <id>`），同一份配置不必在多个任务里各抄一遍：
  ...
  ```
  引用条目**不写** `type` 与 `properties`——字段由预设提供，写出来只会是一份会过期的副本。
  改预设会让所有引用它的任务一起变（文件里的定义在 `/ptxa reload` 后生效）；
  引用不存在的预设、类别用错（拿奖励预设当目标用）、或引用上多写了字段，都会被校验标出来；
  想在某一条上单独调值，就别引用它：把它写成独立配置（显式写出 `type` 与全部字段）。
- 预设的类别取内容里的 `kind: objectives|rewards`；没写 `kind` 时按所在子目录名判断
  （`presets/rewards/money.yml`）；两者都判断不出来就跳过并告警——猜错会把奖励预设混进目标列表；
- 单个文件写坏（缩进、缺引号）**只跳过它并记一条告警**，不会让插件起不来；
  只有注释或内容为空的文件被**静默**忽略（留个空文件写笔记是合理用法）。

`presets/example_mine-stone.yml`：
...
> 预设没有显示名与说明：**文件名（去掉扩展名）就是它的 id**，也是它唯一的身份——
> 游戏内编辑器的预设选择器（字段编辑里的「引用预设」）也是按 id 列的。
> 想单独调值就在那里点「展开为独立配置」。
...
| 情况 | 结果 |
|---|---|
| id 只在文件里 | 生效，但**只读**：游戏内命令改不了它 |
| id 只在数据库里 | 正常，可读可写 |
| 两边都有 | **以数据库为准**，文件里那份被忽略；日志里记一条告警，同一个 id 只报一次 |

插件**只在这两个文件夹空着时写一次示例**，之后再不写入。游戏内命令的保存、
删除一律只改数据库——因此文件里的定义只读（`/ptxa info` 会标出「来源 YAML 文件（只读）」，
`enable` / `disable` 会拒绝并说明原因）。要改它们就改文件，然后 `/ptxa reload`（或重启）；
想搬进数据库就在游戏内照着重建一条，再**把原文件删掉或改名**。
...
默认是 `[MONEY, POINTS]`。常见改法：

```yaml
# 优先扣点券，点券不够才考虑金币（活动期不想消耗玩家金币）
refresh-currency: [POINTS, MONEY]
```

```yaml
# 只扣点券：完全不碰经济插件，也不受 Vault 是否存在影响
refresh-currency: [POINTS]
```
...
- 列表里**没写**的货币视为禁用
- 配的货币在当前服务器上**都不可用**时，退回内置顺序（`[MONEY, POINTS]`）
- 无法识别的名字会被**跳过**并继续往下找，不会因为一个拼写错误整体失效
- 名字大小写不敏感，`points` / `POINTS` 等价
- **两种货币一个都不可用**（既没装经济插件，也没装 PlayerPoints）时，刷新功能直接不可用：
  玩家看到「刷新失败：本服务器没有可用的货币（需要经济插件或 PlayerPoints）」，
  GUI 的刷新按钮上也会写着这句而不是价格。不会白送刷新——那会让配错的 `refresh-cost` 看不出来

### `refresh-cost` 与提示文案

- 设为 `0` 表示免费刷新，此时完全不涉及货币（两个货币插件都没装也能免费刷新）
- 玩家刷新后，提示里会说明**实际扣的是哪种货币**，例如「本次刷新消耗 1,000 金币」或「本次刷新消耗 100 点券」
- 余额不足时提示带货币名与当前余额，例如「点券不足，需要 1000（当前 320）」
- 管理员用 `/ptxa resetperiod` 重置**不扣费也不消耗次数**（它是排障工具，不是消费入口）

> 早期版本有「任务币」内建货币，现已移除：插件不再引入玩家需要额外理解的第二套货币，
> 费用只用上表两种之一。经验也不再是货币——它连奖励类型都不是了，
> 发经验请用命令奖励（`xp add %player% 200 points`），见 [任务奖励](rewards)。
