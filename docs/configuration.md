...
任务定义没有文件形态，改它走两条路，**保存即生效**（不需要重启，也不需要 reload）：

- **网页编辑器**（默认开启，见 [网页编辑器](editor)）：可视化编辑，支持整份任务的导入/导出；
- **游戏内指令**：`/ptxa` 系列（见 [命令](commands)）。

需要 diff、进版本控制或跨服搬运时，用编辑器把任务**导出成 JSON**，整理好再导入。
库里始终是唯一权威，因此不存在「文件和库哪个说了算」的问题。

> 只有在换了存储后端、或在插件外直接改了数据库之后，才需要 `/ptxa reload` 重新载入。
