...
插件把数据分成两类，**各自独立选择存储方式**：

| 数据 | 默认 | 可选 |
|---|---|---|
| **玩家数据**（进度、每日刷新次数） | `SQLITE` | `SQLITE` / `MYSQL` / `JSON` |
| **任务定义与预设** | `JSON` 文件 | `JSON` / `SQLITE` / `MYSQL` |

分开的理由：任务定义是**内容**（人写、改动少、想 diff 与备份），
玩家数据是**状态**（每个游戏事件都可能写、需要事务、多服要共享）。
两者的最优介质本来就不一样。
...
# 任务定义与预设
definitions:
  type: JSON                    # JSON | SQLITE | MYSQL
  folder: "quests"              # type=JSON 时的目录（相对插件数据目录）
```
...
| `JSON` | 一玩家一文件（`players/<uuid>.json`），完全不依赖数据库 |

`JSON` 的取舍要先看清楚：每次进度变化都要重写该玩家的整份文件（实测单玩家约 1KB），
比数据库的一行 upsert 慢，且**文件无法跨服共享**。它只适合「单服 + 玩家不多 +
想彻底不依赖数据库」的场景；默认仍是 SQLite。
...
一个任务一个文件（`quests/<任务id>.json`）：

- **可以直接改**，不必开编辑器；改完 `/ptxa reload` 生效
- **可以 diff 与进版本控制**：改了什么一眼可见
- **可以整目录拷到别的服**（多服时用 `rsync`、git 或压缩包分发，然后各服 reload）
- **改坏一个文件只影响一个任务**：插件会跳过它并在日志里指出文件名，其余任务照常载入

改成 `SQLITE` 或 `MYSQL` 时，任务定义与预设会与玩家数据共用同一个数据库。

> **升级提示**：从旧版本升级、且数据库里已有任务定义时，插件会**自动导出**到
> `quests/` 目录并写明日志。数据库里的旧表**保留不读也不删**，确认无误后可自行清理。

切换后端**不会自动迁移已有数据**（首次启用文件后端那一次除外），需要自行导出导入。

> 玩家数据后端无法打开时插件会被禁用，并在控制台写明原因——避免以「零任务」状态静默运行。
...
1. 读 `plugins/playerTaskX/lang/zh_cn.json`（**推荐离线服手动放一份**）
