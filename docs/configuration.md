...
开启后插件会读取数据目录下的这两个文件夹（不存在时自动创建，**初始是空的**——
里面不会凭空生成示例任务，升级后突然多出几个任务才更可疑）：
...
插件**从不写这两个文件夹**。游戏内命令与网页编辑器的保存、删除一律只改数据库——因此
文件里的定义在编辑器里是只读的（任务列表会标「文件」徽标，任务编辑页顶部有黄色提示条）。
要改它们就改文件，然后 `/ptxa reload`（或重启）；想搬进数据库就在列表页「导出 YAML」，
删掉原文件后再「导入 YAML」。
