...
所有目标类型都支持 `amount`（所需数量），GUI 里会自动列在目标字段的最后。
...
每个目标类型的 `target` 都有自己的**值域**（它能接受哪一类东西），服务端按这份声明校验，
GUI 也按它推图标。写错值域的后果是「配了却永远不命中」，因此这一层刻意做得很紧：
...
- 没装对应插件、或接口签名对不上时，这类目标**永远命中不了**：校验会标出来，
  `/ptxa list`、`/ptxa info` 与启动日志也会写明原因（与 `mythic:` 目标同一套处理）。
...
没装时该类型会被校验标为「不可用」（`/ptxa list` 里能看到），配了也不会涨进度。
...
- `target` 就是你在 CustomFishing 配置里给那条鱼起的 id（大小写不敏感）；
  没装 CustomFishing 时照样能填——那个值在装上之后就会生效
