...
| 49 | **分页基类 `PagedMenu<T>`**：布局图里 `#` 那片格子就是「这一页」（每页条数 = 格子数，不必另配常量）、`prev`/`next`/`pages` 是翻页控件，条目切片与页码夹紧由纯函数 `Paging` 算并单测钉住。插件里三个分页菜单（任务列表、候选选择器、目标/奖励列表）全改用它，各自手写的翻页都删了 | ✅ 完成（YLib 侧 `PagingTest` 4 项；`QuestBrowserMenu` / `CandidateMenu` / `NodeListMenu` 已迁移，第 6 行 6 个按钮 + 3 个空格正好 9 格） |
