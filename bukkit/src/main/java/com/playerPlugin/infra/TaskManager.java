...
public class TaskManager {
    private final List<TaskDefinition> taskDefinitions = new ArrayList<>(); // 存储定义的所有任务 (Task类)
    ...
    public List<TaskTarget> getTaskTargets(String taskId) {
        ...
        return taskDefinition.getTargets();
    }
    ...
    public TaskTrigger getTaskTrigger(String taskId) {
        ...
        return taskDefinition.getTrigger();
    }
    ...
    /**
     * 根据任务ID获取任务（TaskDefinition.class）
     *
     * @param taskId 任务 ID
     * @return {@link TaskDefinition }
     */
    public TaskDefinition getTask(String taskId) {
        return taskDefinitions.stream().filter(task -> task.getId().equals(taskId)).findFirst().orElse(null);
    }

    /**
     * 获取所有任务（TaskDefinition.class）
     *
     * @return {@link Collection }<{@link TaskDefinition }>
     */
    public Collection<TaskDefinition> getAllTasks() {
        return taskDefinitions.stream().toList();
    }
